...
export const handler: APIGatewayProxyHandler = async (event) => {
  ...
  try {
    ...
    const originalPk = article.pk
    const originalSk = article.sk
    ...
    const publishedArticle = {
      ...
      publishDate: now,
      updatedAt: now,
      sk: `PUBLISHED#${article.articleId}`,
    }

    await putArticleRecord(tableName, publishedArticle)

    if (originalSk.startsWith('DRAFT#')) {
      await deleteArticleRecordByKey(tableName, originalPk, originalSk)
    }
    ...
    return successResponse({ article: publishedArticle }, requestId)
  } catch (error) {
    logger.error('Publish article request failed', {
      requestId,
